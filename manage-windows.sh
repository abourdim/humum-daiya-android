#!/usr/bin/env bash
# ============================================================================
#  Humum Daiya Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/26-humum-daiya}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.humumdaiya}"
AVD_NAME="${AVD_NAME:-humum_daiya_test}"
...
cmd_test() {
  ...
  local out="/tmp/humum-daiya-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Humum Daiya Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
