/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Humum Daiya
...
(function() {
  ...
  const NR_T = {
    ...
    fr: {
      ...
      voice: 'Voix', speed: 'Vitesse', pitch: 'Tonalité',
      loop: 'Répéter la carte', sleep: 'Minuterie',
      karaoke: 'Karaoké', autoScroll: 'Défilement auto', duo: 'Duo (AR+FR)',
      off: 'Désactivé', min: 'min',
      bookDone: 'Livre terminé', sleepDone: 'Minuterie terminée', sleepSet: 'Minuterie:',
    }
  };
  ...
  const STATE = {
    ...
    tabOrder: ['about','concerns','solutions'],
    tabIndex: 0,
  ...
  function extractCards(panel) {
    ...
    if (tabName === 'about') {
      ...
      panel.querySelectorAll('.concern-card').forEach(el => {
        if (el.style.display === 'none') return;
        const t = el.querySelector('.concern-title');
        const d = el.querySelector('.concern-desc');
        const s = el.querySelector('.sol-text');
        ...
        cards.push({ el, text: cleanText(text), type: 'concern' });
      });
    } else if (tabName === 'solutions') {
      panel.querySelectorAll('.solution-card').forEach(el => {
        const text = cleanText(el.textContent);
        cards.push({ el, text, type: 'solution' });
      });
    } else if (tabName === 'reflection') {
      panel.querySelectorAll('.reflection-item').forEach(el => {
        const text = cleanText(el.textContent);
        cards.push({ el, text, type: 'reflection' });
      });
  ...
  function cleanText(text) {
    ...
    if (l === 'fr') {
      clean = clean
        ...
        .replace(/Carnegie/gi, 'Carnégi').replace(/Al-Azhar/gi, 'al Azar')
        .replace(/Faisal/gi, 'Faycal').replace(/Humum Da'iya/gi, 'Houmoom Daiya')
    ...
    if (l === 'en') {
      clean = clean
        ...
        .replace(/Humum Da'iya/gi, 'Hoomoom Daa-iya')
  ...
  function readCurrentCard() {
    ...
    if (card.type === 'concern' && card.el && !card.el.classList.contains('open')) card.el.classList.add('open');
    updateProgress();
  ...
  function readDuoTranslation(card, onEnd) {
    ...
    const concernId = card.el ? card.el.id : '';
    const cMatch = concernId.match(/concern-(\d+)/);
    if (cMatch && typeof CONCERNS !== 'undefined') {
      const c = CONCERNS[parseInt(cMatch[1]) - 1];
      if (c && c[duoLang]) duoText = c[duoLang].title + '. ' + c[duoLang].desc;
    }

    if (!duoText && card.type === 'solution' && typeof SOLUTIONS !== 'undefined') {
      const solutionCards = Array.from(card.el.parentNode.querySelectorAll('.solution-card'));
      const idx = solutionCards.indexOf(card.el);
      if (idx >= 0 && SOLUTIONS[idx] && SOLUTIONS[idx][duoLang]) {
        const s = SOLUTIONS[idx][duoLang];
        duoText = (s.title || '') + '. ' + (s.problem || '') + '. ' + (s.solution || '');
      }
    }
  ...
  function pauseNarrator() {
    if (STATE.playing && !STATE.paused) {
      ...
      if ('mediaSession' in navigator) navigator.mediaSession.playbackState = 'paused'; updateUI();
    } else if (STATE.paused) {
      speechSynthesis.resume(); STATE.paused = false;
      if ('mediaSession' in navigator) navigator.mediaSession.playbackState = 'playing'; updateUI();
    }
  }
  ...
  function setupMediaSession() {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      title: l === 'ar' ? 'هموم داعية' : l === 'fr' ? "Soucis d'un Predicateur" : 'Concerns of a Preacher',
  ...
  // ═══ SETTINGS HANDLERS ═══
  function onSpeedChange(val) { STATE.speed = parseFloat(val); const l = document.getElementById('narratorSpeedLabel'); if (l) l.textContent = STATE.speed + 'x'; localStorage.setItem('hd-narrator-speed', STATE.speed); }
  function onPitchChange(val) { STATE.pitch = parseFloat(val); const l = document.getElementById('narratorPitchLabel'); if (l) l.textContent = STATE.pitch.toFixed(1); localStorage.setItem('hd-narrator-pitch', STATE.pitch); }
  ...
  function onKaraokeToggle(checked) { STATE.karaokeEnabled = checked; localStorage.setItem('hd-narrator-karaoke', checked); }
  function onAutoScrollToggle(checked) { STATE.autoScroll = checked; localStorage.setItem('hd-narrator-autoscroll', checked); }
  function onDuoToggle(checked) { STATE.duoReading = checked; localStorage.setItem('hd-narrator-duo', checked); }
  ...
  function onVoiceChange(val) {
    const voices = speechSynthesis.getVoices();
    const voice = voices[parseInt(val)]; if (!voice) return;
    const l = getLang();
  ...
  function loadSettings() {
    STATE.speed = parseFloat(localStorage.getItem('hd-narrator-speed')) || 1;
    STATE.pitch = parseFloat(localStorage.getItem('hd-narrator-pitch')) || 1;
    STATE.karaokeEnabled = localStorage.getItem('hd-narrator-karaoke') !== 'false';
    STATE.autoScroll = localStorage.getItem('hd-narrator-autoscroll') !== 'false';
    STATE.duoReading = localStorage.getItem('hd-narrator-duo') === 'true';
  }
  ...
  function injectSpeakButtons() {
    ...
    var selectors = [
      ...
      '.concern-card', '.solution-card', '.reflection-item', '.daily-card'
    ];
  ...
  var origRenderConcerns = window.renderConcerns;
  if (origRenderConcerns) { window.renderConcerns = function() { origRenderConcerns(); setTimeout(injectSpeakButtons, 100); }; }
  var origRenderSolutions = window.renderSolutions;
  if (origRenderSolutions) { window.renderSolutions = function() { origRenderSolutions(); setTimeout(injectSpeakButtons, 100); }; }
  var origRenderReflection = window.renderReflection;
  if (origRenderReflection) { window.renderReflection = function() { origRenderReflection(); setTimeout(injectSpeakButtons, 100); }; }
